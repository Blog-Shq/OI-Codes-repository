...
#define ctz(x) std::__builtin_ctz(x) //the number of zeros at the end of number
#define clz(x) std::__builtin_clz(x) //the number of zeros at the begin of number
#define popcnt(x) std::__builtin_popcount(x) //the number of ones in the number
#define parity(x) std::__builtin_parity(x) //the parity(odd or even) of the number
...
int main (int argc, char *const argv[]) {
    ...
    while (T --> 0) {
        int j, L, R, z;
        int n = SlowRead();
        up (i, 1, n) l[i][i] = r[i][i] = a[i] = SlowRead();
        up (k, 1, n) up(i, 1, n - k) ((j = i + k, L = l[i][j - 1], R = r[i][j - 1], z = a[j], 1) && ((R == z) && (l[i][j] = 0, 1) || ((L > z && R > z) && (l[i][j] = z, 1) || ((L <= z && R > z) && (l[i][j] = z + 1, 1) || ((L > z && R < z) && (l[i][j] = z - 1, 1) || (l[i][j] = z))))), 1) && ((L = l[i + 1][j], R = r[i + 1][j], z = a[i], 1) && ((R == z) && (r[i][j] = 0, 1) || ((L > z && R > z) && (r[i][j] = z, 1) || ((L <= z && R > z) && (r[i][j] = z + 1, 1) || ((L > z && R < z) && (r[i][j] = z - 1, 1) || (r[i][j] = z))))));
        Print(a[1] != l[2][n]);
        putchar('\n');
    }
    return 0;
